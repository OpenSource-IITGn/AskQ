...
function CommentSection(props) {
  ...
  const handleDeleteComment = (cid) => {
    deleteCommentAndUpdateList(cid);
  };

  const handleSubmit = async () => {
    try {
      const mutation = await createCommentMutation(postId, commentBody);
  ...
  return (
    <FlexboxGrid justify="end" className="comment-section">
      <FlexboxGrid.Item colspan={24}>
        <List size="sm" hover>
          <List.Item className="small-heading">
            <h6>Comments</h6>
          </List.Item>
          {allComments}
